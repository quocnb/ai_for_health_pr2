...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        slice_image = self.data[slc[0]]['image'][slc[1]][None, :]
        slice_seg = self.data[slc[0]]['seg'][slc[1]][None, :]

        sample['image'] = torch.from_numpy(slice_image).type(torch.FloatTensor)
        sample['seg'] = torch.from_numpy(slice_seg).type(torch.LongTensor)
        return sample
