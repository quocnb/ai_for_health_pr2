...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.text((50, 30), "HippoVolume.AI", (255, 255, 255), font=header_font)
    info = (f'''
    ...
    draw.multiline_text((10, 120), info, (255, 255, 255), font=main_font)
    ...
    slc = orig_vol[0, :, :]
    nd_img = np.flip((slc / np.max(slc)) * 0xff).T.astype(np.uint8)
    # This is how you create a PIL image from numpy array
    pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((250, 250))
    # Paste the PIL image into our main report image object (pimg)
    pimg.paste(pil_i, box=(50, 580))
    return pimg
...
if __name__ == "__main__":
    ...
    subdirs = [os.path.join(sys.argv[1], d) for d in os.listdir(sys.argv[1]) if
               os.path.isdir(os.path.join(sys.argv[1], d)) and 'HCropVolume' in d]
